...
func NewMultiTreeExporter(dir string, version uint32, supportExportNonSnapshotVersion bool) (exporter *MultiTreeExporter, err error) {
	...
	if supportExportNonSnapshotVersion {
		db, err = Load(dir, Options{
			...
			WalReaders:          DefaultWalReaders,
			FastStartOpts:       FastStartOptions{FastStartMode: DefaultFastStartMode},
		})
		if err != nil {
		...
		mtree, err = LoadMultiTree(filepath.Join(dir, snapshotName(int64(version))), true, 0, DefaultWalReaders)
		if err != nil {
